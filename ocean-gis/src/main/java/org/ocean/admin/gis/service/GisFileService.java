...
public class GisFileService {
    ...
    public GisImportTaskVO importBatch(Long dataSetId, List<MultipartFile> files) {
        ...
        // 异步任务id
//        String taskId = asyncTaskService.createTask("Gis文件批量导入", files.size(), "UPLOAD", null);
    ...
    private void dispatchImport(
        ...
        try {
            ...
            if (uploadItems.isEmpty()) {
                progressService.finalizeTaskResult(recordNo, "没有可导入的有效文件");
                return;
            }
            ...
            progressService.finalizeTaskFailure(recordNo, "导入任务启动失败: " + ex.getMessage());
            throw ex;
        }
    }
