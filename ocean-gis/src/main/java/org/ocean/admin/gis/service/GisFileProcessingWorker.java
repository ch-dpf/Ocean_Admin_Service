...
public class GisFileProcessingWorker {
    ...
    public void process(GisProcessingExecution execution) {
        try {
            ...
            taskProgressService.updateProgress(
                    execution.taskNo(), 5, "processing", "开始执行切片引擎");
            ...
            taskProgressService.finalizeTaskResult(execution.taskNo(), "切片处理完成");
        } catch (Exception ex) {
            gisTaskService.markFailed(execution.taskId(), ex.getMessage());
            taskProgressService.finalizeTaskFailure(
                    execution.taskNo(), "切片处理失败: " + ex.getMessage());
